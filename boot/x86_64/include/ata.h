#ifndef __UEFI_ATA_H_
#define __UEFI_ATA_H_
...
typedef union {
    UINT8         command_bytes[12];
    UINT16        command_words[6];
} AtapiCommand;
...
static void atapi_detect_capacity(AtaDevice *device)
{
    ...
    // poll
	while (1) {
		UINT8 status = inb(device->io_base + ATA_REG_STATUS);
    ...
    UINT16 capacity_buffer[4];
    UINT8 *capacity_data = (UINT8*)&device->capacity;
    ...
    for(int i = 7; i > -1; i--)
    {
        capacity_data[7 - i] = ((UINT8*) capacity_buffer)[i];
    }
}
...
static void atapi_init_device(AtaDevice *device)
{
    ...
    UINT16 *identity_buffer = (UINT16 *)&device->identity;
...
static int ata_detect_device(AtaDevice *device)
{
    ...
    UINT16 signature = inb(device->io_base + ATA_REG_LBA_MID) | (inb(device->io_base + ATA_REG_LBA_HIGH) << 8);
