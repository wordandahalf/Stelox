...
MKISOFS					:= mkisofs
MKISOFS_FLAGS			:= -quiet -R -J -c boot/bootcat -b boot/boot.img -no-emul-boot -boot-load-size 10 -o $(OS_ISO) -V SteloxCD -input-charset utf-8 -graft-points boot/boot.img=$(OS_FLOPPY) kernel/kernle.elf=$(KERNEL_IMAGE)

QEMU					:= qemu-system-i386
QEMU_FLAGS				:= -d guest_errors -cdrom $(OS_ISO)
